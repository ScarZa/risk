        ...
        <div class="row">
          <div class="col-lg-12">
            ...
            <div class="alert alert-info alert-dismissable">
              ...
              <b>: ความเสี่ยงที่เกี่ยวข้องกับฝ่าย/หน่วยงาน ของเรา</b> 
            </div>
          </div>
        </div><!-- /.row -->
        <form name="form1" action="listRiskInBox.php" method="post" class="navbar-form navbar-right">
 		  ...
 		  <form role="search" action='session_search.php' method='post'  >
		       <div class="form-group input-group">
		    		<input type='search' name='take_detail' placeholder='ค้นหาความเสี่ยง...' value='' class="form-control" > 
				      <input type='hidden' name='method'  value='take_detail'> 
...
function page_navigator($before_p,$plus_p,$total,$total_p,$chk_page){   
	...
	$urlfile="listRiskInBox.php?status_process=$status_process"; // ส่วนของไฟล์เรียกใช้งาน ด้วย ajax (ajax_dat.php)
	$per_page=10;
...
if(!empty($_REQUEST['status_process'])){ 
$status_process=$_REQUEST['status_process'];
...
//คำสั่งค้นหา
     $q="select  *,LEFT(take_detail,100) AS detail  from   takerisk t1
         ...
         Where t1.recycle='N' and s1.name like '%$Search%'  and t1.res_dep='$user_dep' and t1.move_status='N' $where_code01"; 
 }else{
 $q="select  * ,LEFT(take_detail,100) AS detail from   takerisk t1
     $join_code
     inner join subcategory s1 on t1.subcategory=s1.subcategory
     Where t1.recycle='N' and  t1.res_dep='$user_dep' and t1.move_status='N' $where_code01 ORDER BY t1.takerisk_id DESC "; 
...
?>  
 					<TR >	    
                                            ...
                                            <TD><center><a href="detailRiskInBox.php?takerisk_id=<?=$result[takerisk_id]?>&status_process=<?= $status_process?>"  <?=$class?> ><?=$result[takerisk_id]?> </a></center> </TD>
                                            <TD><a href="detailRiskInBox.php?takerisk_id=<?=$result[takerisk_id]?>&status_process=<?= $status_process?>"  <?=$class?> ><?=$result[name]; ?> </a> </TD>
                                            <TD ><center><a href="detailRiskInBox.php?takerisk_id=<?=$result[takerisk_id]?>&status_process=<?= $status_process?>"  <?=$class?> > <?php echo DateThai1($take_date);?></a></center></TD>
                                            <TD ><center><a href="detailRiskInBox.php?takerisk_id=<?=$result[takerisk_id]?>&status_process=<?= $status_process?>"  <?=$class?> > <?php echo DateThai1($take_rec_date);?></a></center></TD>
                                            <TD ><center><a href="detailRiskInBox.php?takerisk_id=<?=$result[takerisk_id]?>&status_process=<?= $status_process?>"  <?=$class?> ><?php echo $status;?></a></center></TD>					 
					</TR> 
