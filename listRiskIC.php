...
function page_navigator($before_p,$plus_p,$total,$total_p,$chk_page){   
 ...
 if($_SESSION[ic]!=''){
    $Search=trim($_SESSION[ic]);
 echo "<br><br><br>แสดงคำที่ค้นหา : ".$Search;
//คำสั่งค้นหา
     $q="select  *,LEFT(take_detail,100) AS detail  from   takerisk    Where  take_detail like '%$Search%'  and ic='Y' "; 
 }else{
 $q="select  * ,LEFT(take_detail,100) AS detail from   takerisk Where   ic='Y' ORDER BY takerisk_id DESC "; 
...
*/
	if($bg == "#F4F4F4") { //ส่วนของการ สลับสี 
		...
		$bg = "#F4F4F4";
								...
								where m1.takerisk_id='$takerisk_id' AND t1.ic='Y' ");
