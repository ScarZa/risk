...
<!-- ค้นหา -->
  <form class="navbar-form navbar-left" role="search" action='session_search.php' method='post'  >
       <div class="form-group">
		<input type='text' name='Search_place' placeholder='สถานที่'  value='' class="form-control"  > 
		<input type='hidden' name='method'  value='search_place'> 
		 </div>
...
function page_navigator($before_p,$plus_p,$total,$total_p,$chk_page){   
 ...
 $Search_place=trim($_SESSION[Search_place]);
 if($Search_place!='' ){
 
  echo "แสดงคำที่ค้นหา : ".$Search_place;
//คำสั่งค้นหา
     $q="select * from place 
 	  Where  name like '%$Search_place%'  "; 
 }else{
 $q="select * from place "; 
 }
...
<body>
 ...
 <TR bgcolor='#C3C3C3'>
					...
					<th width='80%'><CENTER>สถานที่</CENTER></th>
...
?>  
					...
					<TD><?=$result[name]; ?></TD>
 					<TD><CENTER>
				    <a href='frmPlace.php?method=update&place=<?=$result[place]?>' ><i class="fa fa-edit"></i></a> 
				    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					
					<a href='./prcPlace.php?method=delete&place=<?=$result[place]?>'  title="confirm" onclick="if(confirm('ยืนยันการลบ <?php  echo  $result[name]; ?>&nbsp;ออกจากรายการ ')) return true; else return false;">   
