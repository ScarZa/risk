...
<form method="post" action="" enctype="multipart/form-data" class="navbar-form navbar-right">
 ...
 if(empty($_POST[take_date1]) and empty($_POST[take_date2])){
 if($date >= $bdate and $date <= $edate){
   		...
   		$mng_status=$_POST[mng_status];							include_once ('funcDateThai.php');
...
if($take_date1!='' and $mng_status==''){  
	...
	echo "<p>&nbsp;</p></center>";
				 ...
				 where    t1.move_status='N' and t1.take_date between '$take_date1' and '$take_date2' group by t1.level_risk order by t1.level_risk   ");	
	...
	echo "<p>&nbsp;</p></center>";
 				$sql = mysql_query("select count(m1.mngrisk_id) as number_risk,t1.level_risk from mngrisk m1
				 LEFT OUTER JOIN takerisk t1 on t1.takerisk_id = m1.takerisk_id
 				 where   m1.mng_status='$mng_status' and t1.take_date between '$take_date1' and '$take_date2' and t1.move_status='N'   group by t1.level_risk order by t1.level_risk  ");	
	...
	echo "<p>&nbsp;</p></center>";
 				...
 				$sql = mysql_query("select count(m1.mngrisk_id) as number_risk,t1.level_risk from mngrisk m1
				 LEFT OUTER JOIN takerisk t1 on t1.takerisk_id = m1.takerisk_id
 				 where   m1.mng_status='$mng_status' and t1.move_status='N' and t1.take_date between '$take_date1' and '$take_date2' group by t1.level_risk order by t1.level_risk  ");	
...
<tr bgcolor=<?=$bg?> onmouseover="mOvr(this,'#DEF8F0');" onclick=mClk(this); onmouseout=mOut(this); >
	<TD><CENTER>
 	...
 	<a href='session_search.php?level_risk=<?=$rs[level_risk]?>&&method=level_risk&mng_status=<?=$mng_status?>&&take_date1=<?=$take_date1?>&&take_date2=<?=$take_date2?>&&type=1' title='ดูรายงานแยกหน่วยงาน' target='_blank'><?php echo $i; ?></CENTER></a></TD>
