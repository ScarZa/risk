...
if($take_date1!='' and $mng_status==''){  
	...
	echo "<p>&nbsp;</p></center>";
$sql=mysql_query("select count(mngrisk_id) as number_risk,d1.name as dep_name ,c1.name as category_name,c1.category ,t1.level_risk ,t1.subcategory ,t1.take_dep ,s1.name as sub_name
				 from mngrisk m1
				 ...
				 where  t1.level_risk='$level_risk' and t1.take_dep='$take_dep' and t1.category='$category' and  t1.move_status='N' and t1.take_date between '$take_date1' and '$take_date2' group by t1.subcategory order by s1.subcategory ASC");	
...
$sql=mysql_query("select count(mngrisk_id) as number_risk,d1.name as dep_name ,c1.name as category_name,c1.category ,t1.level_risk,t1.subcategory ,t1.take_dep ,s1.name as sub_name
				 ...
				 LEFT OUTER JOIN subcategory s1 on s1.subcategory = t1.subcategory
				 LEFT OUTER JOIN level_risk r1 on r1.level_risk = t1.level_risk
 				 where  t1.level_risk='$level_risk' and t1.take_dep='$take_dep' and t1.category='$category' and m1.mng_status='$mng_status' and t1.move_status='N'   group by t1.subcategory  order by s1.subcategory ASC");	
...
,t1.take_dep   ,s1.name as sub_name
				 from mngrisk m1
				 LEFT OUTER JOIN takerisk t1 on t1.takerisk_id = m1.takerisk_id
				 LEFT OUTER JOIN department d1 on t1.take_dep = d1.dep_id
				 LEFT OUTER JOIN category c1 on c1.category = t1.category
				 LEFT OUTER JOIN subcategory s1 on s1.subcategory = t1.subcategory
				 LEFT OUTER JOIN level_risk r1 on r1.level_risk = t1.level_risk
 				 where t1.level_risk='$level_risk' and t1.take_dep='$take_dep' and t1.category='$category' and m1.mng_status='$mng_status' and t1.move_status='N' and t1.take_date between '$take_date1' and '$take_date2' group by t1.subcategory  order by s1.subcategory ASC ");	
	...
	echo "<p>&nbsp;</p></center>";
				 ...
				 LEFT OUTER JOIN subcategory s1 on s1.subcategory = t1.subcategory
				 LEFT OUTER JOIN level_risk r1 on r1.level_risk = t1.level_risk
				 where t1.level_risk='$level_risk' and t1.take_dep='$take_dep' and t1.category='$category' and t1.move_status='N' group by t1.subcategory order by s1.subcategory ASC");
 }		  
...
<tr bgcolor=<?=$bg?> onmouseover="mOvr(this,'#DEF8F0');" onclick=mClk(this); onmouseout=mOut(this); >
	...
	<TD><CENTER>
            <a href='session_search.php?method=main_report&subcategory=<?=$rs[subcategory]?>&&level_risk=<?=$rs[level_risk]?>&&take_dep=<?=$rs[take_dep]?>&&category=<?=$rs[category]?>&&mng_status=<?=$mng_status?>&&take_date1=<?=$take_date1?>&&take_date2=<?=$take_date2?>&&type=1' title='ดูรายงานเรื่องนี้'><?php echo $i; ?></a></CENTER></TD>
	<TD><a href='session_search.php?method=main_report&subcategory=<?=$rs[subcategory]?>&&level_risk=<?=$rs[level_risk]?>&&take_dep=<?=$rs[take_dep]?>&&category=<?=$rs[category]?>&&mng_status=<?=$mng_status?>&&take_date1=<?=$take_date1?>&&take_date2=<?=$take_date2?>&&type=1' title='ดูรายงานเรื่องนี้'><?php echo $rs[sub_name];?></a></TD>
	<TD><a href='session_search.php?method=main_report&subcategory=<?=$rs[subcategory]?>&&level_risk=<?=$rs[level_risk]?>&&take_dep=<?=$rs[take_dep]?>&&category=<?=$rs[category]?>&&mng_status=<?=$mng_status?>&&take_date1=<?=$take_date1?>&&take_date2=<?=$take_date2?>&&type=1' title='ดูรายงานเรื่องนี้'><p align="right"><?php echo $rs[number_risk];?></p></a></TD>
    <?php }else{?>
