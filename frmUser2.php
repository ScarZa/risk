    ...
    <div class="col-lg-6"> 
        <div class="alert alert-success">
			...
			<div class="form-group">
                            <?php mysql_close($con); ?> 
                            <?php
                            $con2 = mysql_connect("localhost","rploeigo_hrd","gotoytoynoy")or die("can not connect database");
                            $db2 = mysql_select_db("rploeigo_hrd",$con2)or die("can not select database");
                            mysql_query("SET NAMES 'utf8'", $con2); ?>
                            
			<label>ชื่อ</label>
<select name="user_fname" id="user_fname" required  class="form-control"  onkeydown="return nextbox(event, 'fname');"> 
				<?php	$sql = mysql_query("SELECT empno,firstname  FROM emppersonal order by empno ");
				 echo "<option value=''>เลือกชื่อ</option>";
				 while( $result = mysql_fetch_assoc( $sql ) ){
          if($result[empno]==$resultGet[Name]){$selected='selected';}else{$selected='';}
				 echo "<option value='$result[firstname]' $selected>$result[firstname] </option>";
				 } ?>
			 </select>
                            
                                <label>นามสกุล</label>
<select name="user_lname" id="user_lname" required  class="form-control"  onkeydown="return nextbox(event, 'fname');"> 
				<?php	$sql = mysql_query("SELECT empno,lastname  FROM emppersonal order by empno ");
				 echo "<option value=''>เลือกนามสกุล</option>";
				 while( $result = mysql_fetch_assoc( $sql ) ){
          if($result[empno]==$resultGet[Name]){$selected='selected';}else{$selected='';}
				 echo "<option value='$result[lastname]' $selected>$result[lastname] </option>";
				 } ?>
			 </select>
