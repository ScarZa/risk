...
<?php include 'header.php';?>
		<?php 
         ...
         ?>
<H1><small>รายงานแสดงภาพรวมของความเสี่ยงที่ได้รับทั้งหมด</small></H1>
...
$sql=mysql_query("select count(mngrisk_id) as number_risk,d1.name as dep_name ,c1.name as category_name,c1.category  from mngrisk m1
				 ...
				 where   t1.res_dep='$user_dep_id' and  t1.move_status='N' and t1.take_date between '$take_date1' and '$take_date2' group by c1.category order by number_risk DESC");	
...
$sql=mysql_query("select count(mngrisk_id) as number_risk,d1.name as dep_name ,c1.name as category_name,c1.category  from mngrisk m1
				 ...
				 LEFT OUTER JOIN department d1 on t1.res_dep = d1.dep_id
				 LEFT OUTER JOIN category c1 on c1.category = t1.category
 				 where    t1.res_dep='$user_dep_id' and t1.take_date between '$date_start' and '$date_end' and  m1.mng_status='$mng_status' and t1.move_status='N'   group by c1.category order by number_risk DESC");	
...
$sql=mysql_query("select count(mngrisk_id) as number_risk,d1.name as dep_name ,c1.name as category_name,c1.category  from mngrisk m1
				 ...
				 LEFT OUTER JOIN department d1 on t1.res_dep = d1.dep_id
				 LEFT OUTER JOIN category c1 on c1.category = t1.category
 				 where    t1.res_dep='$user_dep_id' and  m1.mng_status='$mng_status' and t1.move_status='N' and t1.take_date between '$take_date1' and '$take_date2' group by c1.category order by number_risk DESC");	
	...
	echo "<p>&nbsp;</p></center>";
				 ...
				 LEFT OUTER JOIN department d1 on t1.res_dep = d1.dep_id
				 LEFT OUTER JOIN category c1 on c1.category = t1.category
				 where    t1.res_dep='$user_dep_id' and t1.take_date between '$date_start' and '$date_end' and t1.move_status='N' group by c1.category order by number_risk DESC");
...
<tr bgcolor=<?=$bg?> onmouseover="mOvr(this,'#DEF8F0');" onclick=mClk(this); onmouseout=mOut(this); >
	<TD><CENTER>
 	...
 	<a href='main_report3_2.php?take_dep=<?=$user_dep_id?>&category=<?=$rs[category]?>&&mng_status=<?=$mng_status?>&&take_date1=<?=$take_date1?>&&take_date2=<?=$take_date2?>&&type=1' title='ดูรายงานแยกหน่วยงาน'><?php echo $i; ?></CENTER></a></TD>
	<TD><a href='main_report3_2.php?take_dep=<?=$user_dep_id?>&category=<?=$rs[category]?>&&mng_status=<?=$mng_status?>&&take_date1=<?=$take_date1?>&&take_date2=<?=$take_date2?>&&type=1' title='ดูรายงานแยกหน่วยงาน'><?php echo $rs[category_name];?></a></TD>
	<TD><a href='main_report3_2.php?take_dep=<?=$user_dep_id?>&category=<?=$rs[category]?>&&mng_status=<?=$mng_status?>&&take_date1=<?=$take_date1?>&&take_date2=<?=$take_date2?>&&type=1' title='ดูรายงานแยกหน่วยงาน'><p align="right"><?php echo $rs[number_risk];?></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</p></TD>
</TR>
