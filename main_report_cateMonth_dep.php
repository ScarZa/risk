...
if($category=='' and $year!=''){
 ...
 $month_end= $month_end.$i="$year-01-31";		
		...
		if($category=='' and $year!=''){ 
		 ...
		 from takerisk t1  
						 LEFT OUTER JOIN category c1 on c1.category = t1.category
						 where t1.take_dep='$user_dep_id' and  t1.category='$category'  and t1.take_date between '$month_start' and '$month_end'  and t1.move_status='N' order by number_risk DESC");	
		 }else if($category=='' and $year==''){ 
 		 $sql=mysql_query("select count(takerisk_id) as number_risk from takerisk t1  
						 LEFT OUTER JOIN category c1 on c1.category = t1.category
						 where     t1.take_dep='$user_dep_id' and t1.take_date between '$month_start' and '$month_end'  and t1.move_status='N' order by number_risk DESC");	
...
<TR>
 ...
 $I = 10;$c=1;
		 ...
		 $sql=mysql_query("select count(takerisk_id) as number_risk from takerisk t1  
						 LEFT OUTER JOIN category c1 on c1.category = t1.category
						 where t1.take_dep='$user_dep_id' and  t1.category='$category'    group by c1.category and t1.move_status='N' order by number_risk DESC");	
		 }else if($category=='' and $year!=''){
		 $sql=mysql_query("select count(takerisk_id) as number_risk ,c1.category
		 from takerisk t1  
						 LEFT OUTER JOIN category c1 on c1.category = t1.category
